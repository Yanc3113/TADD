...
#             # 检查索引是否在有效范围内
#             max_index = self.args.clip_visfea_sampleNum - 1
#             if interpolated_indices.max() > max_index or interpolated_indices.min() < 0:
#                 raise IndexError(f"Interpolated indices out of range. Max index: {interpolated_indices.max()}, "
#                                 f"Min index: {interpolated_indices.min()}, Allowed range: [0, {max_index}]")
...
#             else:
#                 # 尝试加载文件并索引
#                 visual_fea_path = osp.join(self.CLIP_visual_fea_dir, video_id + '.pt').replace("\\", "/")
...
def video_loader(video_dir_path, frame_indices, image_loader):
    video = []
    tmpl = 'frame_{:06d}.png'  # Modify template to match frame naming format
    for i in frame_indices:
        image_path = os.path.join(video_dir_path, tmpl.format(i)).replace("\\", "/")
        if os.path.exists(image_path):
            video.append(image_loader(image_path))
        else:
            print(f"Frame does not exist: {image_path}")
            return video  # If frame does not exist, return loaded part
    return video
